...
import h5py
import numpy as np
#from files import  interpolate
import hdf5fileinout as hdf5io
import sys
from utils import getCerenkovAngle
...
def extractData(sim_file, primary, energy, zenith, azimuth, injection_height, altitude):
    ...
    for i in range(n):
        ...
        a = np.zeros([len(Traces[:,0]), 4])
        a[:,0], a[:,1], a[:,2], a[:,3] = Traces[:,i], Traces[:,i+n], Traces[:,i+2*n], Traces[:,i+3*n]  
        
        #np.savetxt("a%d.trace" %i, a )
    ...
    return (Primary, Energy, Zenith, Azimuth, GroundAltitude, Positions, Traces, XmaxPosition, XmaxDistance, Distplane)    
    
def _scale_run(Traces, Positions, primary, E1, zen1, az1, injh1, dist1, E2, zen2, az2, injh2, altitude):
    ...
    for l in np.arange(0, end):
        ...
        # positions
        txt3, pos_new[l, :] = _scalingpulse(Traces, dist1, E1, az1, zen1, injh1, E2, az2,
                                            zen2, injh2, primary, phigeo,
                                            thetageo, l,  positions, altitude)
    
        #print(txt3)

    np.savetxt("newpos.txt", pos_new)


    #### includes the savings!!!
...
def _getXmax(primarytype, energy, zen2):
    ...
    if primarytype == 'electron':  # aprroximated by gamma shower
        a = 82.5  # g/cm2
        c = 342.5  # g/cm2
    if primarytype == 'Proton':  # aprroximated by proton
        a = 62.5  # g/cm2
        c = 357.5  # g/cm2
...
def _scalingpulse(Traces, dist1, E1, az1, zen1, injh1, E2, az2, zen2, injh2, primary, phigeo, thetageo, l,  positions, altitude):
    '''Returns factors to scale the amplitude 
    ...
    #print(dist1, E1, az1, zen1, injh1, E2, az2, zen2, injh2, primary, phigeo, thetageo, l,)
# SCALING factorss
  ...
  ################################################
    ...
    try:
        ...
        txt1 = np.zeros([len(Traces[:,0]), 4])
        n = 176
        txt1[:,0], txt1[:,1], txt1[:,2], txt1[:,3] = Traces[:,l], Traces[:,l+n], Traces[:,l+2*n], Traces[:,l+3*n]
    except IOError:
...
def myscale(sim_file, primary, energy, zenith, azimuth, injection_height, altitude):
    
    
    (primary, E1, zen1, az1, GroundAltitude, Positions, Traces, XmaxPosition, XmaxDistance, dist1) = extractData(sim_file, primary, energy, zenith, azimuth, injection_height, altitude)
    injh1 =2000
    _scale_run(Traces, Positions, primary, E1, zen1, az1, injh1, dist1,
                       energy, zenith, azimuth, injection_height, altitude)
